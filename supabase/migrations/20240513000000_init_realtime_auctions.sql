...
BEGIN;
  ...
  -- Bids: Everyone can view, only the system (via place_bid) or the bidder can manage
  CREATE POLICY "Bids are viewable by everyone" ON public.bids FOR SELECT USING (true);
  -- In Supabase, RLS policies are bypassable by service_role, and place_bid runs with definer rights (see below)

  -- Function to place a bid with server-side validation
  -- SECURITY DEFINER ensures this runs with the privileges of the creator (admin/system)
  ...
  END;
  $$ LANGUAGE plpgsql SECURITY DEFINER; -- Important for RLS bypass during bid placement
