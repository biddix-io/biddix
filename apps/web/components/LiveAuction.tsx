      ...
      <div className="p-6 md:p-8">
        <div className="flex justify-between items-start mb-6">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="inline-block px-2 py-1 rounded bg-primary/10 text-primary text-xs font-bold uppercase tracking-wider">
              ...
              <div className="flex items-center gap-1.5 ml-2">
                ...
                <span className="text-[10px] text-muted-foreground uppercase tracking-widest">{onlineBidders} online</span>
              </div>
            </div>
        ...
        <div className="border-t border-border pt-6">
          ...
          <div className="space-y-3">
            {bids.map((bid) => (
              <div key={bid.id} className={`flex justify-between items-center py-2 border-b border-border/50 last:border-0 ${bid.bidder_id === userId ? 'bg-primary/5 px-2 rounded' : ''}`}>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-[10px] font-bold">
                    {bid.bidder_id === userId ? 'YOU' : 'ID'}
                  </div>
                  <div>
                    <p className="text-sm font-medium">
                      {bid.bidder_id === userId ? 'You' : `User ${bid.bidder_id.slice(0, 8)}`}
                    </p>
